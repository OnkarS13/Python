...
# You must have noticed that till now we have been learning one new concept per tutorial. 
# For some important concepts like loops we had to allocate two tutorials so we can grasp the concept of both loops (for and while) separately. 
# But now in the case of the file, we have allocated the next five tutorials (excluding the exercise and their solutions). 
# So, from this, you can take a hint that how important file handling is in programming.

# In this tutorial we are not getting into files in detail, instead, we are discussing the basics of the file and its modes in a theoretical manner. 
# In computer terms, “a file is a resource for saving data and information in computer hardware”. A file is stored in the form of bytes in hardware. 
...
# Unlike C or C++, file handling in python is relatively easy and simple.  Python treats files differently as text or binary and this is important. 
...
# We can understand by the name of the text file that it must contain text in it. 
# The extension for the text file is .txt. All other forms of files are mostly binary even a .doc file, that we open in Microsoft Word is a 
# binary file because it requires special software for accessing it.

# The second sort of files are binary files. They are almost all the other files that we come in contact with while using our computer. 
# These files include images, PDFs, Excel files, etc.
