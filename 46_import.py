...
# In this tutorial we aim to understand the working of the import statement, so we can have a better grasp of the concept and resolve common importing issues. 
# In Python, we give access to a module by using a keyword import. To use any package in our code, we must make it accessible by importing it first. 
# There are many ways we can import a module in python, but what can be easier than using a single keyword, so it is also the most common way for importing.
...
# A common mistake that most of the beginners do and also the primary reason for making this tutorial is, why can’t we name our file, the same as the name of a module. 
# The reason is associated with the path. When we give our file a name same as the name of a module, then instead of importing the original module, 
...
# The first one is to import using an object. For this, we usually import the whole module by using a simple import statement. When we use only the import keyword, 
...
# We can not access it using pandas keyword; instead, we have to use pd or the compiler will show an error. 
# This case comes in handy when the module name is difficult or lengthy, and we have to use a module again and again to call its functions.
