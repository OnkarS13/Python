...
# This statement uses the % operator. Similarly, in Python, we can perform string formatting using the % operator. For Example:
...
# The problem with this method is when we have to deal with large strings. If we specify the wrong type of input type operator, 
# then it will throw an error. For Example, %d will throw a TypeError if the input is not an integer.
...
# Tuples are better than using the old formatting string method. However, it is not an ideal way to deal with large strings. For Example:
...
# for string formatting more regular. str.format() allows multiple substitutions and value formatting. 
...
# A much more simple and intuitive solution is the use of Formatted string literals.f-string has an easy syntax as compared to previous string 
# formatting techniques of Python. They are indicated by an "f" before the first quotation mark of a string. Put the expression inside { } 
# to evaluate the result. Here is a simple example
...
# b = a.format(me, a1)
# print(b)
a = f"this is {me} {a1} {4 * 95} {math.cos(65)}"
# time
print(a)
