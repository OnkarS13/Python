...
# As we have studied function in the previous lecture, we know that if a program has a large piece of code that is required to be executed repeatedly, 
# then it will be better to implement that piece of code as a function. Functions improve code reusability, modularity, and integrity.
...
# The function can then be called using this variable. In the above code, the function has been called with three different parameter values 
...
# We can create anonymous functions wherever they are needed. Due to this reason, Python Lambda Functions are also called as 
# throw-away functions which are used along with other predefined functions such as reduce(), filter(), and map(). 
